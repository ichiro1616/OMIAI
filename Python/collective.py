            ...
            for i in range(player):  # キー：player_id
                while(counter == 0):
                    if(flug == 0):
                        ...
                        cur.execute(
                            "SELECT MAX(`generation`) FROM `collective` WHERE `rotation` = %s AND `experience_years` = %s AND `player_id` = %s;", (Dcount, h, i+1))
                        ...
                        # 世代の値がある場合はテーブルに入っていた世代の次の世代から計算をする
                            ...
                            clip = (gene*5)
                            flug = 1
                    length = len(data[Dcount][(data[Dcount]['ex_years'] == h) & (
                        data[Dcount]['player_id'] == i+1)]['x'])-clip
                    ...
                    if((length < 5) & (length != 0)):  # 現在取得したデータを5つずつ区切って残りデータが5より少なくなった場合はループから抜ける
                        ...
                        x_tmp = ((data[Dcount][(data[Dcount]['ex_years'] == h) & (
                            data[Dcount]['player_id'] == i+1)]['x'][0:clip]).values.tolist())
                        y_tmp = ((data[Dcount][(data[Dcount]['ex_years'] == h) & (
                            data[Dcount]['player_id'] == i+1)]['y'][0:clip]).values.tolist())
                        ...
                        print("y_mean = ", y)
                        cur.execute(
                            'INSERT INTO `collective`(`rotation`, `generation`, `experience_years`,`player_id`, `x_coordinate`, `y_coordinate`) VALUES (%s,%s,%s,%s,%s,%s);', (Dcount, gene, h, i+1, x, y))
                        conn.commit()
                        print("rotation=", Dcount, "experience_years=", h, "player_id=",
                              i+1, "x_coordinate=", x, "y_coordinate=", y, "generation=", gene)
                        gene += 1
                        time.sleep(0.2)
                counter = 0
                gene = 0
