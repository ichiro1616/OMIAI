import numpy as np
import pandas as pd
# コートを上から見たときのボール座標を取得するために、2台のカメラで撮った動画から変換式を連立させ、スクリーン座標からワールド座標へ変換します
...
def transformation(csv, filename, path):
    ...
    for index, row in csv.iterrows():
        if index == 0:
            A = np.array([[-fx1, 0., -Cx1 + row['offset_usiro_x']],
                          [Cy2 - row['offset_yoko_y'], fy1 +
                              fy2, -Cy1 + row['offset_usiro_y']],
                          [Cx2 - row['offset_yoko_x'], 0., -fx2]])
            b = np.array([(-row['offset_usiro_x'] + Cx1) * tz1, (-row['offset_usiro_y'] + Cy1)
                         * tz1 + (row['offset_yoko_y'] - Cy2) * tz2, (row['offset_yoko_x'] - Cx2) * tz2])
        ...
        b = np.array([(-row['usiro_x'] + Cx1) * tz1, (-row['usiro_y'] + Cy1) *
                     tz1 + (row['yoko_y'] - Cy2) * tz2, (row['yoko_x'] - Cx2) * tz2])    #
    ...
    csv.to_csv(path + '\\trans_' + filename,
